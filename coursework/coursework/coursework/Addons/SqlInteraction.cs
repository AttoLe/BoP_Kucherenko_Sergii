...
public static class SqlInteraction
{
    private static readonly SqlConnection Connection =
        new (@"Server=SERHII\MSSQLSERVER2; Database=coursework; Integrated Security = True");
    ...
    public static T ExecuteScalar<T>(string com) =>
        (T)  CreateCommand(com).ExecuteScalar();
    
    public static List<T> GetList<T>(string com, List<object> values)
    {
        List<T> data = new List<T>();
        SqlCommand command = CreateFunction(com, values);
